...
import type { JobFormData } from '@/features/admin/components/forms/JobForm';
import { JobForm } from '@/features/admin/components/forms/JobForm';
import { jobService } from '@/features/admin/services/apiService';
...
export default function AdminJobs() {
  ...
  const [editingItem, setEditingItem] = useState<JobFormData | null>(null);
  const [deletingItem, setDeletingItem] = useState<JobFormData | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const loadData = async () => {
    setIsLoading(true);
    const items = await jobService.getAll();
    setData(items);
    setIsLoading(false);
  };
  ...
  const handleSave = async (formData: JobFormData) => {
    ...
    try {
      if (editingItem?.id) {
        await jobService.update(editingItem.id, formData);
      } else {
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await jobService.delete(deletingItem.id);
  ...
  const columns = [
    ...
    { header: 'Type', accessor: 'employmentType' as const },
    {
      header: 'Actions',
      accessor: 'id' as const,
      cell: (item: JobFormData) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-content mb-1">Job Postings</h1>
          <p className="text-sm text-content-secondary">Manage open positions and career opportunities.</p>
        </div>
      ...
      <AdminModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); }} title={editingItem ? 'Edit Job' : 'Post New Job'}>
        <JobForm initialData={editingItem} onSubmit={handleSave} onCancel={() => { setIsModalOpen(false); }} isSubmitting={isSubmitting} />
      </AdminModal>
