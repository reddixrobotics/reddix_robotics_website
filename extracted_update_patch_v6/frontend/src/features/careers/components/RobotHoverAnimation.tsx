...
export function RobotHoverAnimation({ className, style }: RobotHoverAnimationProps) {
  ...
  const drawFrame = (index: number, img?: HTMLImageElement) => {
    ...
    if (!imageToDraw || !imageToDraw.complete) {
      // Find the closest loaded image to prevent flickering while loading
      let found = false;
      for (let offset = 1; offset < FRAME_COUNT; offset++) {
        const prev = index - offset;
        const next = index + offset;
        if (prev > 0 && imagesRef.current[prev]?.complete) {
          imageToDraw = imagesRef.current[prev];
          found = true;
          break;
        }
        if (next <= FRAME_COUNT && imagesRef.current[next]?.complete) {
          imageToDraw = imagesRef.current[next];
          found = true;
          break;
        }
      }
