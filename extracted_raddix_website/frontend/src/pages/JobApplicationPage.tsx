      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {job ? (
            ...
            <div className="lg:w-2/3">
              <div className="mb-8 border-b border-[var(--border-strong)] pb-8">
                ...
                <div className="flex flex-wrap items-center gap-4 text-body-md text-[var(--text-secondary)]">
                  <span className="text-[var(--color-brand)] font-medium bg-[var(--color-brand)]/10 px-3 py-1 rounded-full">{job.department}</span>
                  {job.type && <Badge variant="neutral">{job.type}</Badge>}
                  {job.location && <div className="flex items-center gap-2"><MapPin size={18} /> {job.location}</div>}
                  {job.experienceLevel && <div className="flex items-center gap-2"><Briefcase size={18} /> {job.experienceLevel} Experience</div>}
                  {job.salary && <div className="flex items-center gap-2"><Briefcase size={18} /> {job.salary}</div>}
                </div>
              </div>
              ...
              <div className="prose prose-invert max-w-none text-body-lg text-[var(--text-secondary)]">
                ...
                <p className="whitespace-pre-line mb-10 leading-relaxed text-[var(--text-secondary)]">{job.description}</p>
                
                {job.requirements && job.requirements.length > 0 && (
                  <>
                    <h3 className="text-2xl font-bold text-white mb-4 mt-10">Requirements & Skills</h3>
                    <ul className="list-disc pl-5 space-y-3 mb-8 text-[var(--text-secondary)]">
                      {job.requirements.map((req: string) => (
                        <li key={req} className="pl-2 leading-relaxed">{req}</li>
                      ))}
                    </ul>
                  </>
                )}
            ...
            <div className="lg:w-1/3 w-full lg:sticky lg:top-24">
              <Card className="p-6 md:p-8 border-[var(--border-strong)] bg-[var(--bg-secondary)]/50 backdrop-blur-sm shadow-xl rounded-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <InputField label="Portfolio URL" type="url" placeholder="https://" />
          ...
          <div className="text-center py-20">
             ...
             <h1 className="text-xl text-[var(--text-secondary)]">Loading Job Details...</h1>
          </div>
        )}
        ...
        <Modal isOpen={showModal} onClose={handleClose}>
          ...
          <ModalBody>
            <div className="py-4 text-center">
              ...
              <p className="text-[var(--text-secondary)]">Your application for <strong>{job?.title}</strong> has been submitted successfully. We will be in touch soon!</p>
            </div>
          </ModalBody>
          <ModalFooter>
